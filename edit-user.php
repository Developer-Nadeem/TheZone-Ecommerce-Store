...
if(isset($_POST['submitted'])) {
    if (!empty(trim($_POST['editFirstName'])) && !empty(trim($_POST['editLastName']))  && !empty(trim($_POST['editUserEmail'])) && !empty(trim($_POST['editUserPassword'])) && isset($_SESSION['CSRF_Token'])&& isset($_SESSION['isAdmin']) && $_SESSION['CSRF_Token'] === $_POST['csrftoken']) {
        try {
            ...
            $updateUser = $db->prepare("UPDATE useraccounts SET Firstname=:firstname, Lastname=:lastname, Email=:userEmail, Pass=:userPassword, isAdmin=:updateAdmin WHERE UserID = :userID");
            ...
            $updateUser->bindParam(':updateAdmin', $isAdmin);
